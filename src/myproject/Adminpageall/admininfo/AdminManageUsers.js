...
function AdminManageUsers() {
  ...
  const deleteUser = useCallback(async (customerId) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`${ADMIN_API}/delete/${customerId}`, { timeout: 10000 });
  ...
  const handleBulkApprove = useCallback(async () => {
    ...
    try {
      const approvePromises = Array.from(selectedUsers).map(userId =>
        axios.put(`${ADMIN_API}/approve/${userId}`, {}, { timeout: 10000 })
      );
  ...
  const handleBulkReject = useCallback(async () => {
    ...
    try {
      const rejectPromises = Array.from(selectedUsers).map(userId =>
        axios.put(`${ADMIN_API}/reject/${userId}`, {}, { timeout: 10000 })
      );
